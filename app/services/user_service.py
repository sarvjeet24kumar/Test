...
class UserService:
    ...
    async def create_user(
        ...
        if not target_tenant_id and role != UserRole.SUPER_ADMIN:
             raise ForbiddenException("Tenant ID is required for non-Super Admin users")
        ...
        if result.scalar_one_or_none():
            raise ConflictException(f"Username '{data.username}' already exists in this tenant")
        ...
        if result.scalar_one_or_none():
            raise ConflictException(
                f"Email '{data.email}' already exists in this tenant or as a global admin"
            )
    ...
    async def get_user(self, user_id: UUID, requester: User) -> User:
        ...
        if requester.role == UserRole.SUPER_ADMIN:
            if user.role != UserRole.TENANT_ADMIN:
                raise ForbiddenException("SuperAdmin can only access Tenant Admin accounts")
            return user
            
        # 3. TenantAdmin check: Same tenant
        if requester.role == UserRole.TENANT_ADMIN:
            if user.tenant_id != requester.tenant_id:
                raise ForbiddenException("Tenant Admin can only access users in their own tenant")
            return user
    ...
    async def update_user(
        ...
        if requester.id == user.id:
            if data.is_active is not None or data.deleted_at is not None:
                raise ForbiddenException("You cannot modify your own account status (active/deleted)")
